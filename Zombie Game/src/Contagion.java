...
public class Contagion {
    ...
    public Contagion()
    {
    	...
    	rng = new Random();		//Temporary rng for testing purposes; don't believe it's seeded with system time at the moment.
    }
    

	/*		Last edited 3/11/21 Bryan Polk
	The disease fighter calculator takes a person and checks for disease. This calculator is currently able to randomize damage to the
	infected person based on the disease's lethality and the person's immune system resistance, and updates the disease's lifespan variable
	after iterating to reflect the time passing.
	*/
	...
	/*		Last edited 3/11/21 Bryan Polk
	The disease spread calculator considers the spread between two members of a MinorGroup. It checks to see if members of a group have any
	diseases to spread, then simulates the possibility of an infected individual contacting an at risk individual. Furthermore, it simulates
	the possible spread of the virus to the at risk individual, and will update their disease parameter as needed.	
	*/
	public void diseaseSpreadCalculator(MinorGroup group)
	{
		...
		for(int i = 0; i < group.getPeople().size(); i++)
		{
			...
			{
				//upon finding an infected group member, check interaction with all at risk group members
				for(int j = 0; j < group.getPeople().size(); j++)
				{
					if(!group.getPeople().get(j).getHasDisease())
					{
						...
						if(rng.nextFloat() < group.getPeople().get(i).getExposureLevel()*group.getPeople().get(j).getExposureLevel())
						{
							//If the random number is smaller than the product of the peoples' exposure levels, they interact.
							if(rng.nextFloat() < group.getPeople().get(i).getDisease().getTransmissionRange()*group.getPeople().get(j).getImmuneSystemResistance())
							{
								//If the random number is smaller than the product of the transmission value of the disease and the at risk person's immune system
	...
	/*		Last edited 3/11/21 Bryan Polk
	The response threshold calculator considers the given group's exposure level and response threshold, and if the threshold is met
	will reduce the baseline exposure of all group members based on the innate response strength value of the group.
	It will then update the value of the baseline exposure to reflect the changes.
	*/
	public void responseThresholdCalculator(MinorGroup group)
	{
		...
		{
			//reduce all group members' exposure levels in accordance with the response strength of the group
			for(int i = 0; i < group.getPeople().size(); i++)
			{
				group.getPeople().get(i).setExposureLevel(group.getPeople().get(i).getExposureLevel()-group.getResponseStrength());
			}
			group.setBaselinePopulationExposure(calculateBaselineExposure(group));
		}
	}
	
	/*		Last edited 3/06/21 Bryan Polk
 	This function calculates a group's average exposure level across all Person objects it contains.	
	*/
